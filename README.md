...
- **主要機能**: 無料の回答表示、支払い（疑似）後に詳細を表示するフロー（疑似 402 → 200）。
- **表示モード**: 本アプリは「通常モード」と「モックモード」を持ちます。URL の `?mock=true` またはヘッダーのリンクで切替可能で、ページ上部に固定のモードバナーが表示されます。モードに応じて `body` に `mode-demo` / `mode-mock` クラスが付与され、背景色・アクセントが切り替わります。
...
- モックで起動: ブラウザで [index.html](index.html) を開き、URL に `?mock=true` を付与して開くか、画面右上の「モックモードへ」リンクを押してください。
- 挙動: モックモードでは実際のウォレットや MetaMask を使わず、画面上にモック支払いダイアログが表示されます。モックでの承認を行うと擬似的な `payer` と `txHash` が生成され、アプリは支払い済みとしてワークフローを進めます。モード中はヘッダーに「通常モードへ」リンクが表示され、これで通常モードに戻せます。
