...
- **主要機能**: 無料の回答表示、支払い（疑似）後に詳細を表示するフロー（疑似 402 → 200）。
- **表示モード**: 本アプリは「通常モード」「モックモード」「AIモード」の3種類を持ちます。ヘッダー右上のモード選択（セレクト）で切替できます（順序: 通常モード → モックモード → AIモード）。また、URL に `?mock=true` を付与して起動することも可能です。選択に応じてページ上部に固定のモードバナーが表示され、`body` に `mode-demo` / `mode-mock` / `mode-ai` のクラスが付与され、背景色・アクセントが切り替わります。AIモードではバナーに「AIモード（調整中）」と表示され、文字色は黒になります。
...
- **主要ファイル**:
  ...
  - `sample.html` — タイトル/配色サンプル（デザイン確認用）
...
- モックで起動: ブラウザで [index.html](index.html) を開き、URL に `?mock=true` を付与するか、ヘッダー右上のモード選択（セレクト）で「モックモード」を選んでください。
- 挙動: モックモードでは実際のウォレットや MetaMask を使わず、画面上にモック支払いダイアログが表示されます。モックでの承認を行うと擬似的な `payer` と `txHash` が生成され、アプリは支払い済みとしてワークフローを進めます。モックモードから通常モードに戻すには、ヘッダーのモード選択で「通常モード」を選んでください。
- 注意: モックはあくまでデモ用です。本番環境では `?mock=true` を付けずに運用してください。
